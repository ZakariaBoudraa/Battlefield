...
public class Map {
    ...
    // 1 represents the warrior castle
    ...
    public boolean build() {
        ...
        int index = 1;
        //find the start point (Castle tile)
        for (int i=0; i<mapLayout.length; i++) {
            for (int j=0; j<mapLayout[i].length; j++) {
                if (mapLayout[i][j] == 1) {
                    tiles[i][j] = new Tile();
                    castleTile = tiles[i][j];
                    castleTile.buildCastle();
                    startX = i;
                    startY = j;
                    break;
                }
        ...
        pathTiles.remove(pathTiles.size() - 1);	//remove last because the last one is CampTile
        campTile = tiles[startX][startY];
        campTile.buildCamp();
        campTile.createPath(lastTile, null);
        return true;
    }
